...
**Important:** Do not commit real credentials. Use environment variables (e.g. `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`) or base64-encoded placeholders in local config only.

```typescript
const decode = (s: string) => Buffer.from(s, "base64").toString();

const CLIENT_ID = process.env.GOOGLE_OAUTH_CLIENT_ID ?? decode("<BASE64_CLIENT_ID>");
const CLIENT_SECRET = process.env.GOOGLE_OAUTH_CLIENT_SECRET ?? decode("<BASE64_CLIENT_SECRET>");
```
